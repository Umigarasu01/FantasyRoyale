...
- 仮Player表示に本番`CharacterActor2D`、本番`CharacterHealth`、Actor所有のRigidbody2D / 足元Capsuleが接続される。
- Runtime複製された`Player/Move`がWASD Bindingを持ち、仮想Keyboard入力が移動Commandへ届く。
- 人間入力AdapterがMove Actionを純C# Commandへ変換し、Actorが同じCommandを適用する。
- 仮Playerが空き方向へ移動する。
...
- Reference SceneのEventSocket 6点が固定定義なしの`PoolCandidate`であり、個別半径を持つ。
- Preview Seedから3地点だけが有効化され、抽選済み定義がPoolとCatalogから同じ泉参照へ解決される。
- 泉の回復成功時だけHPが増え、Socket ID単位のOneShot状態がDictionaryへ保存される。
- HP満タン時は失敗結果となり、Socketを使用済みにしない。
- HP 0の本番Character Healthでは通常回復を拒否し、Socketを使用済みにせず、撃破理由をHUDへ表示する。
- HP 0のActorへ移動Commandを渡しても位置が変わらず、Commandが停止へ戻る。
- Registryへ泉Handlerが一件登録され、実Scene経路が`Succeeded` / `Rejected`を返す。
- 成功時は回復Message / Effect / Audio Cue、満タン時は拒否MessageだけをPresenterへ渡す。
...
- [Scene Builder](battle-royale-exploration-preview-debug-scene-builder.md)

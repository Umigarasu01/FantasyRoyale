...
Poolは「何を何件抽選できるか」だけを保存する。「どのSocketへ何が出たか」は[MapEventPlacementSelector](map-event-placement-selector.md)の結果であり、Match終了時に破棄またはMatch状態として別保存する。

Event定義自体は[MapEventDefinition](../../map-authoring/map-event-definition.md)を参照し、全定義のID検索正本は`MapEventCatalog`を維持する。

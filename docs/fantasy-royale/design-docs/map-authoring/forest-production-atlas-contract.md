...
正本を参照してbuilt-in `imagegen`で生成したTallGrass、FlowerPatch、Wildflowers、Reedsの4画像を、v4.3の自由配置Decoration Sourceとする。地面の矩形を含めず、Magenta Keyを透明化し、足元中央PivotでPrefab化する。保存先、プロンプト概要、Hash、元出力は[Generated Assets](../../development/assets/generated-assets.md)を正とする。
...
ForestWallには共通Socket Crop `(x=20, y=16, width=110, height=110)` を適用し、向かい合うSocketのOpaque重なり12px以上、実測最小で東西13px・南北15pxを確認していた。この値とv3 Atlas / Catalog / QAのHash、18 passedという結果は履歴であり、v4.3の完了根拠にはしない。詳細は[Generated Assets](../../development/assets/generated-assets.md)を参照する。
